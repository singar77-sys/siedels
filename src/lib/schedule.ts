...
/**
 * Serializable version of the week schedule for passing to client components.
 * Converts the Map in each day to a plain object.
 */
export interface SerializableDaySchedule {
  dayIndex: number;
  dayName: string;
  date: string | null;
  shopHours: string;
...
export interface SerializableWeekSchedule {
  days: SerializableDaySchedule[];
  isCurrent: boolean;
  stale: boolean;
  fetchedAt: string;
}

export function toSerializable(week: WeekSchedule): SerializableWeekSchedule {
  return {
    days: week.days.map((d) => {
      const shifts: Record<string, Shift> = {};
      d.shifts.forEach((s, n) => (shifts[n] = s));
      return {
        dayIndex: d.dayIndex,
        dayName: d.dayName,
        date: d.date,
        shopHours: d.shopHours,
        shifts,
      };
    }),
